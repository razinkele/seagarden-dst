...
|---|---|---|
| Site conditions | `forcing.PLACEHOLDER_SITES` | **The live instance runs on a real artifact since 2026-09-25.** The first annual refresh built it on laguna on 2026-09-24 (v0.11.2, 2016–2025, 39.9 % of cells valid) and the service reads it through `SEAGARDEN_DATA_DIR`; the Site panel banner says so. What remains placeholder: any region without a coordinate (EE-coastal, LT-coastal), and **LT-lagoon**, whose sited coordinate lands in a cell the Baltic reanalysis masks (nearest valid cell 1.74 km away) — the app shows that with the coverage note, and re-siting it is a decision for the pilot, not for the code. The placeholder table itself is unchanged and still what a checkout without an artifact runs on: at Tagalaht its DIN is **5.3× high** (5.5 against a measured 1.05 µmol/L) and attenuation **2.0× high** (0.4 against 0.198), while salinity is good. Values deliberately left uncorrected. `docs/2026-09-15-package-b-measurements.md`, `docs/runbooks/annual-refresh.md` |
...
| Light attenuation | `SiteConditions.light_attenuation_k`, default `0.4` | Wired on the artifact path: the refresh derives k = 1.7/z_SD from daily Secchi depth (`zsd`, Poole–Atkins) and averages it monthly, so an artifact-backed site reads a **derived** value (0.13–0.19 at the four coastal placeholder sites on the 2026-09-24 build, against the 0.4 default) — derived, not measured, which affects the tier a result inherits. The 0.4 default still applies on the placeholder path. Measured 0.198 at Tagalaht. `docs/2026-09-15-package-b-measurements.md` §5 |
| Map and polygon drawing | `modules/site.py` | Same, plus `shiny_deckgl` — a conda prerequisite, not a pip dependency: `micromamba install -n shiny -c razinka shiny-deckgl` |
