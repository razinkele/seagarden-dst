...
- **R1 — the baseline window is stashed during `build()`, for the three YEARLY layers only.** `baseline_years()` takes no argument and `REGISTRY` holds instances constructed at import, so a yearly layer learns its window from the `YearRange` handed to `build()`. `driver.py:160` calls `build()` before `driver.py:174` calls `baseline_years()`, so this works today. `copernicus_phy`, `copernicus_bgc` and `copernicus_bgc_light` each store the range on `self` in `build()` and **raise `RuntimeError` if `baseline_years()` is called first** — the ordering becomes enforced and tested rather than implicit.
  **`copernicus_wav` is exempt, and the exemption is the point.** Its p95 window is fixed at 2023–2025 by C§3.2, so it is a design constant rather than a function of the request, and it can answer before `build()` has run. Read correctly, `wav` is the layer behaving as C-b's handoff note wanted every layer to behave — *"a real layer must know its baseline window without having built anything"* — and the guard on the other three is a compromise forced on them by a window they genuinely cannot know until told. A constraint, not a virtue: do not add the guard to `wav` for symmetry. (This corrects R1's original wording, which said "each layer" and made the correct case look like the exception.)
...
### Deferred from C-c1's whole-branch review — carried here so they survive the scratch directory

**1. The probe hits the wrong kind of URL, and two statements about it are now false.** All four layers `HEAD` an HTML *product landing page*. `copernicus_bgc` and `copernicus_bgc_light` share a `PRODUCT_ID`, so their probe URLs are byte-identical and the probe cannot tell the monthly dataset from the daily one — "one layer, one dataset" does not hold on the probe path. Worse, a landing page returns 200 long after a `dataset_id` is retired or a version superseded, which is precisely the event the monthly probe exists to catch. Meanwhile `.github/workflows/source-probe.yml` still passes `COPERNICUSMARINE_SERVICE_USERNAME`/`PASSWORD` that nothing reads, and its comment claims *"probe() makes catalogue metadata calls only"*, which a stdlib `HEAD` is not. C§8.2 specifies catalogue metadata calls and an institutional credential; **C-c1's R4 narrowed that to stdlib HTTP without recording the deviation anywhere.** Decide: either probe something that can 404 per dataset (check whether `copernicusmarine`'s catalogue lookup has an endpoint reachable with stdlib and no credential, and whether a bogus `dataset_id` 404s there), or amend C§8.2 and the workflow's comment and env block to match what R4 forces. Either way, write the decision down — a future reader should not have to infer it from a comment that contradicts the code.

**2. Real coordinates versus `GridSpec`, before trusting `check_grid`.** Nothing in C-c1 reindexes Copernicus output onto the grid: `open_window` sends a lat/lon bounding box, and `check_grid` then `np.allclose`es the merged coordinates against `GridSpec.baltic().lats()/lons()` — which are **lower cell edges** starting exactly at 53.5/9.5 with truncated steps (0.016666 against 1/60). Over 390 rows the truncation alone drifts ~2.6e-4°. Whether the CMEMS subset's native centres land inside `allclose`'s tolerance, and whether WAV shares the PHY/BGC grid closely enough for `xr.merge(join="exact")`, is unverified and unverifiable offline. This is a C-b design tension — edge-versus-centre coordinates plus an exact-equality check plus unregridded sources — not a C-c1 defect, but the measured thin slice is where it surfaces.

**3. Does the daily BGC dataset accept a depth window?** C-c1's kwarg check verified parameter *names* against `copernicusmarine.open_dataset`, not that every dataset accepts every one. `copernicus_wav` opts out with `surface=False` on the argument that the wave product is 2-D and handing it a depth window is an error rather than a no-op. If `zsd` is likewise 2-D in that request, `bgc_light` has the same problem and needs the same treatment. `drop_depth` is defensive enough to survive either outcome, so this is a first-real-call risk rather than a code bug.

**4. `REGISTRY` holds stateful singletons constructed at import.** `_years` is set by `build()` and never reset, and the CLI hands `list(REGISTRY.values())` straight to `run_refresh`. Harmless today — one refresh per process, and `build()` overwrites — but a partially-failed refresh leaves layers holding a window from the failed run, and any future test that builds a registry layer leaks state into every later test. Consider a dict of factories when the registry tightens to `==`.

**5. `copernicus_wav`'s laziness is the one path still unproven.** C-c1 added a test pinning that a dask-backed source survives `phy.build()` still chunked. `wav`'s `groupby().quantile()` on a dask array is the reduction that actually matters — it is the ~25.8 GB one — and no test exercises it. The rechunk guard at `wav.py`'s `chunk({"time": -1})` was also found to be unnecessary in the installed dask/xarray, so it is insurance rather than a demonstrated requirement.

**6. Smaller, already recorded:** `url_reachable`'s non-2xx branch is untested; its `except` clause enumerates exception types rather than catching everything, so a layer's `probe()` would propagate an unexpected error type; and only `copernicus_wav` asserts what it actually asked Copernicus for — the three yearly layers never verify that they pass the requested range or the surface depth window through.
