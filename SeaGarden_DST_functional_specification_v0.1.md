...
| Revision | 13 September 2026 — implementation language changed from R to Python (§3.2). Consequential changes in §8.1–8.3, §13, §14. Decision **D1 resolved**: *Chorda filum* ships in v1 on assumed analogue parameters (§5.3, §11). |
...
| Layer | Choice | Why |
|---|---|---|
| Application | **Shiny for Python** (`shiny`, `shinywidgets`) | Python is KU MRI's working language for this class of tool — pymarxan, the HELCOM WMS viewer, bowtiepy, the EUTROPY/OSMOPY chain — so the DST is maintainable by the same people and reusable from the same workflows. Shiny's reactive model suits a tool whose outputs all recompute from a drawn polygon and a few selectors. |
| Spatial | `rioxarray` / `xarray` (rasters), `geopandas` / `shapely` (vectors), `pyproj`, `ipyleaflet` via `shinywidgets` (map and polygon drawing) | Covers everything §5 needs — zonal statistics, overlay, reprojection, interactive draw — with no service dependency. |
| Model core | Pip-installable package `seagarden_dst` (`numpy`, `scipy`, `pandas`) | Pure functions, no UI imports. Testable with `pytest`, callable from a notebook or a batch script, and importable into other KU MRI model chains. |
...
| Raster storage | Cloud-optimised GeoTIFF, pre-clipped to the South Baltic, read with `rasterio`/`rioxarray` | No PostGIS, no tile server. |
| Deployment | Docker image, `uvicorn` ASGI server, nginx reverse proxy, KU MRI server | One `docker compose up` rebuild. Sits alongside the existing laguna.ku.lt tooling. |
| Dependencies | `pyproject.toml` with a committed lockfile; no conda | A decade-old environment has to be reconstructible from the repository alone. |
...
- **Environmental summary** — salinity, sea surface temperature, DIN and DIP concentration, photosynthetically active radiation, depth, and significant wave height, each as a seasonal climatology with range (§6).
